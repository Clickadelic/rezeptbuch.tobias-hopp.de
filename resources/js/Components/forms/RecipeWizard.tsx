...
export default function RecipeCreateWizard({ recipe, ingredients, className }: RecipeCreateWizardProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className={cn('flex flex-col space-y-6', className)}>
      ...
      <ol className="flex items-center justify-between text-sm">
        <li className={cn('flex-1', step >= 1 ? 'font-semibold text-primary' : 'text-slate-400')}>1 · Basics</li>
        <li className={cn('flex-1 text-center', step >= 2 ? 'font-semibold text-primary' : 'text-slate-400')}>2 · Zutaten</li>
        <li className={cn('flex-1 text-right', step >= 3 ? 'font-semibold text-primary' : 'text-slate-400')}>3 · Bilder & Abschluss</li>
      </ol>
      {!canNextFromStep1 && (
        <div className="border border-sky-400 bg-sky-200 text-sm text-sky-700 px-4 py-3 rounded relative" role="alert">
            <p className="text-smmt-2">Bitte bei Name mindestens 3 Zeichen und Beschreibung mindestens 10 Zeichen ausfüllen.</p>
        </div>
      )}
      {/* STEP 1: Basics */}
      {step === 1 && (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="flex items-end">
                ...
                />
                <span className="px-3 py-2 border border-l-0 rounded-r-lg border-gray-400">Minuten</span>
              </div>
              <Slider
          ...
          <div className="w-64 mx-auto flex justify-center">
            <Button type="button" onClick={() => setStep(2)} disabled={!canNextFromStep1}>
              Weiter
            </Button>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          {data.recipe_ingredients?.map((di, idx) => (
            <div key={idx} className="flex flex-row justify-start gap-2 items-start mb-3">
              ...
              <Select value={di.unit} onValueChange={(value) => updateIngredient(idx, 'unit', value)}>
      ...
      {step === 3 && (
        <section className="space-y-5">
          ...
          {/* Zubereitung */}
          <div className="w-full space-y-3">
            <InputLabel htmlFor="preparation_instructions" value="Zubereitung, so geht's…" />
            <Textarea
