    ...
    return (
        ...
        >
            <h3
                className={cn(
                    'text-lg flex gap-2',
                    comments && comments?.data?.length >= 1 && 'mb-3',
                )}
            >
                {icon}
                {title || 'Deine Kommentare'}
            </h3>
            ...
            {(!comments || comments.data.length === 0) && (
                <div className="h-[calc(100%-25px)] flex flex-col gap-2 items-center justify-center">
                    <h4 className="text-gray-600 dark:text-gray-400 text-center mb-2">
                        Du hast noch keine Kommenare hinterlassen.
                    </h4>
                    <Button asChild variant="primary" className="hover:bg-emerald-700">
                        <Link href={route('comments.index')} title="Erstelle einen Kommentar">
                            <GoPlus /> Kommentar erstellen
                        </Link>
                    </Button>
                </div>
            )}
