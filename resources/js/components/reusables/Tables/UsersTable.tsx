...
export default function UsersTable({ title, icon, className, users }: UserListProps) {
  return (
    <div className={cn('w-full bg-gray-100 dark:bg-gray-900 p-4 rounded-xl', className)}>
      <h3 className={cn("text-lg flex gap-2", users && users?.length >= 1 && "mb-3")}>{icon}{title || 'Deine Favoriten'}</h3>
      {/* Wenn keine User vorhanden */}
      {(!users || users.length === 0) && (
        <div className="h-[calc(100%-25px)] flex flex-col gap-2 items-center justify-center">
            <h3 className="text-gray-600 dark:text-gray-400 text-center mb-2">Es sind noch keine User vorhanden.</h3>
        ...
        <Table className="border-collapse table-auto caption-bottom">
          ...
          <TableBody>
            {users.map((user: AuthUser) => (
              <TableRow key={user.id} className="hover:bg-white dark:hover:bg-gray-900">
                <TableCell className="cursor-default">
                  asd
                </TableCell>
                <TableCell className="cursor-default truncate max-w-[150px]">{user.name}</TableCell>
