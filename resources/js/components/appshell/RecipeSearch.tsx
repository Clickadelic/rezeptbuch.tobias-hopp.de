        ...
        <div className={cn("relative w-full h-48 lg:h-72 bg-[url('../images/Cocktail-at-the-pool.jpg')] dark:bg-[url('../images/brooke-lark-M4E7X3z80PQ-unsplash-cut.jpg')] bg-cover bg-center flex flex-col justify-between items-center")}>
            ...
            <div className="w-full h-1 backdrop backdrop-blur"></div>
                ...
                <div className={cn("bg-white/30 dark:bg-gray-800/30 flex flex-col p-1 rounded mx-4 my-6 animate ease-in-out transition-all duration-200", isBlurred ? 'lg:w-[32rem]' : 'lg:w-[28rem]')}>
                    ...
                    <ul className={cn("p-0 overflow-hidden place-items-center items-center justify-center grid grid-cols-6 z-30 h-0 animate transistion-all duration-200 ease bg-white dark:bg-gray-800 rounded-bl rounded-br", isBlurred && "h-[76px] p-1 gap-1")}>
                        <li><Link href={route('recipes.search', { search: 'Vorspeise' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zu den Vorspeisen"><TbSalad className="size-6" />Vorspeisen</Link></li>
                        <li><Link href={route('recipes.search', { search: 'Hauptgericht' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zu den Hauptgängen"><PiCookingPot className="size-6" />Hauptgang</Link></li>
                        <li><Link href={route('recipes.search', { search: 'Nachtisch' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zum Nachtisch"><RiCake3Line className="size-6" />Nachtisch</Link></li>
                        <li><Link href={route('recipes.search', { search: 'Cocktail' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zu den Cocktails"><LiaCocktailSolid className="size-6" />Cocktails</Link></li>
                        <li><Link href={route('recipes.search', { search: 'Backen' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zum Backen"><GiCakeSlice className="size-6" />Backen</Link></li>
                        <li><Link href={route('recipes.search', { search: 'Snack' })} className={cn("flex text-gray-600 hover:text-primary p-3 items-center justify-center text-xs sm:text-sm flex-col gap-1")} title="Zu den Snacks"><GiCrystalBars className="size-6" />Snack</Link></li>
                    </ul>
                </div>
